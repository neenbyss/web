'use client';
import * as React from 'react';

import { cn } from '@/lib/utils';
import { AlertIcon } from '@/icons/alert';

export type TextareaProps = {
  label?: string;
  errorMessage?: string;
  startContent?: React.JSX.Element;
  inputClassName?: string;
  classNames?: {
    container?: string;
    input?: string;
  };
} & React.ComponentProps<'textarea'>;

export const Textarea: React.FC<TextareaProps> = ({
  ref,
  className,
  label,
  startContent,
  errorMessage,
  classNames,
  onChange,
  ...props
}) => {
  const [focused, setFocused] = React.useState(false);
  const [length, setLength] = React.useState(props.value?.toString().length);

  const textareaRef = React.useRef<HTMLTextAreaElement>(null);

  const resizeTextArea = () => {
    if (textareaRef.current) {
      textareaRef.current.style.height = 'auto';
      textareaRef.current.style.height = textareaRef.current.scrollHeight + 'px';
    }
  };

  const handleLength = React.useCallback((value: number) => {
    setLength(value);
  }, []);

  const onFocus = () => {
    if (!focused) {
      setFocused(true);
      textareaRef.current?.focus();
    }
  };

  const onBlur = () => setFocused(false);

  const handleChange = React.useCallback(
    (t: React.ChangeEvent<HTMLTextAreaElement>) => {
      if (props.readOnly || props.disabled) {
        return;
      }
      if (onChange) {
        onChange(t);
      }
      if (props.maxLength) {
        handleLength(t.target.value.length);
      }
      resizeTextArea(); // Llama a resizeTextArea aquí
    },
    [handleLength, onChange, props.disabled, props.maxLength, props.readOnly],
  );

  React.useImperativeHandle(
    ref,
    () =>
      ({
        focus: onFocus,
        focused: focused,
        ...textareaRef.current,
      }) as HTMLTextAreaElement,
  );

  React.useEffect(() => {
    resizeTextArea(); // Asegúrate de que la altura se ajuste al montar el componente
  }, []);

  return (
    <div
      className={cn(
        'w-full',
        props.disabled && 'pointer-events-none opacity-60',
        classNames?.container,
      )}
    >
      {label && (
        <label className='mb-1 flex w-full max-w-full items-end justify-between gap-3 overflow-hidden'>
          {' '}
          <span className='flex max-w-full gap-1 overflow-hidden'>
            <span className='text-foreground truncate'>{label}</span>
            {props.required && <span className='text-danger'> * </span>}
          </span>{' '}
          {props.maxLength && (
            <span className='text-foreground/50 text-xs text-nowrap'>
              {' '}
              {length} / {props.maxLength}{' '}
            </span>
          )}
        </label>
      )}
      <div
        onClick={() => {
          textareaRef.current?.focus();
        }}
        className={cn(
          'rounded-md transition-all duration-150',
          'flex gap-2',
          'bg-default hover:bg-default/80 w-full max-w-full px-3 py-2',
          'outline-ring outline-1',
          'data-[focus=true]:bg-default data-[focus=true]:ring-primary/50 data-[focus=true]:ring-2 data-[focus=true]:outline-none',
          'data-[error=true]:!outline-danger/20 data-[error=true]:data-[focus=true]:!outline-danger [&_*]:data-[error=true]:!text-danger',
          className,
        )}
        data-disabled={props.disabled}
        data-focus={focused}
        data-error={Boolean(errorMessage)}
        onFocus={onFocus}
        onBlur={onBlur}
      >
        {startContent}
        <textarea
          ref={textareaRef}
          className={cn(
            'placeholder:text-foreground/50 data-[error=true]:placeholder:!text-danger/60 w-full resize-none bg-transparent text-sm transition outline-none',
            'max-h-64 min-h-20',
            props.disabled && 'pointer-events-none',
            classNames?.input,
          )}
          onChange={handleChange}
          data-error={Boolean(errorMessage)}
          {...props}
        />
      </div>
      {errorMessage && (
        <span className='text-danger mt-1.5 flex items-center gap-1.5 text-xs font-normal'>
          {' '}
          <AlertIcon className='size-3.5' /> {errorMessage}{' '}
        </span>
      )}
    </div>
  );
};
